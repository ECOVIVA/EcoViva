...
class UsersTest(APITestCase, UsersMixin):
    ...
    def test_get_user_profile_fail_for_unauthorized(self):
        ...
        self.assertEqual(
            response.json().get("detail"), "As credenciais de autenticação não foram fornecidas."
        )
    ...
    def test_post_user_create_fail_for_photo_invalid_for_type(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.json().get("photo")[0],
            "A extensão de arquivo “gif” não é permitida. As extensões válidas são: jpg, jpeg, png .",
        )
    ...
    def test_patch_user_update(self):
        ...
        payload = {"username": "newuser", "phone": "12345678910"}
    ...
    def test_patch_user_update_with_interest(self):
        ...
        payload = {"username": "newuser", "phone": "12345678910", "interests": ["Reciclagem"]}
    ...
    def test_patch_user_update_fail_for_username_duplicate(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.json().get("username")[0], "Um usuário com este nome de usuário já existe."
        )
